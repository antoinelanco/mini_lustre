...
let schedule_equs inputs equs =
  ...
  let rec exists_loop topo g =
    if Graph.is_empty g then List.rev topo
      ...
      let sv = Graph.fold (fun (x,_,_) s -> S.add x s) g1 S.empty
      in
      let g = Graph.fold (fun (y,s,e) g ->
          Graph.add (y,S.diff s sv,e) g
        ) g2 Graph.empty
      in
      let topo = Graph.fold (fun (_,_,e) l ->
...
let schedule_node n =
  let equs = schedule_equs n.tn_input n.tn_equs in { n with tn_equs = equs }
